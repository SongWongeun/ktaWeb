...
<%
	...
	String passwd = encrypt("SHA1", request.getParameter("passwd"));
	String webPageKind = request.getParameter("webPageKind");
	String result_sc = "";
	...
	if(id.equals(u_id)){
		if(passwd.equals(u_pass)){
			...
			result_sc = "<script>alert(ce_bh.ce_kT); history.back();</script>";
		}
	}
	else{
		result_sc = "<script>alert(ce_bh.ce_tn); history.back();</script>";
	}
