...
<%
	...
	String userAddMenuList [] = request.getParameter("UserAddMenu").split("\\|\\|");
	...
	for(int i=0;i<parent.size();i++){
		...
		for(int j=0;j<children.size();j++){
			...
			if(children.get(j).equals("AddMenu")){
				...
				if(count == 0){
					for(int k=0; k < userAddMenuList.length;k++){
 						xmlText += "		<" + children.get(j) + ">" + userAddMenuList[k] + "</" + children.get(j) + ">\n";
					}
					count = 1;
				}
				
			}
			else if(request.getParameter((String)children.get(j)) == null){	
				xmlText += "		<" + children.get(j) + "></" + children.get(j) + ">\n";
			}
			else{ 
				xmlText += "		<" + children.get(j) + ">" + request.getParameter((String)children.get(j)) + "</" + children.get(j) + ">\n";
			}
		}
		xmlText += "	</" + parent.get(i) + ">\n";
	}
	...
	if(check){
		result_sc = "<script type='text/javascript' language='javascript'>alert(ce_bh.ce_lq); window.document.location.href='manager_setting.jsp?Tab=" + request.getParameter("Tab") + "';</script>";
	}
	else{
		result_sc = "<script>alert(ce_bh.ce_iB+'" + fileName + "');  history.back();</script> ";
	}
