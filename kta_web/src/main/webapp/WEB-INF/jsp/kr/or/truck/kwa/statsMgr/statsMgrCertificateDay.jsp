...
<script src="/bootStrap/javascripts/application.js" type="text/javascript"></script>
 ...
 <style>
        #xchart-statistics1 .tip .text, #xchart-statistics2 .tip .text, #xchart-statistics3 .tip .text, #xchart-statistics4 .tip .text {
            background: #465866;
            color: #fff;
            padding: 5px 10px;
       ...
       #xchart-statistics1 .tip .arrow, #xchart-statistics2 .tip .arrow, #xchart-statistics3 .tip .arrow, #xchart-statistics4 .tip .arrow {
       	height:6px;
...
$(document).ready(function(){
	...
	$('#sts_year .year').each(function(){
		$(this).on('click',function(){
			...
			var y = $(this).attr('y');
			location.href = '/kwa/statsMgr/certificate/year.do?year='+y;
		});
	});
	...
	$('#sts_month .year').each(function(){
		$(this).on('click',function(){
			...
			var y = $(this).attr('y');
			location.href = '/kwa/statsMgr/certificate/month.do?year='+y;
		});
	});
	/* end:월별통계 버튼설정 */
	/* start:일별통계 버튼설정 */
	...
	$('#sts_day .month').each(function(){
		$(this).on('click',function(){
			...
			var m = $(this).attr('m');
			location.href = '/kwa/statsMgr/certificate/day.do?year='+y+'&month='+m;
		});
	});
...
<div class="container" style="max-width: none;width:1300px;padding:0px;">
			...
			<div class="box">
				<div class="box-header">
					<span class="title" style="font-size:15px;font-weight:bolder;text-align:left;background-color:rgb(255, 243, 218);color:darkgoldenrod;width:100%;">
						분담급 납입증명서 ( <%=reqYear %>년 <%=reqMonth %>월 통계 )
					</span>
				</div>
				<div class="box-content padded">
		...
		<div class="col-md-12 padded"> 
			<div class="box">
				<div class="box-header">
					<span class="title" style="font-size:15px;font-weight:bolder;text-align:left;background-color:rgb(255, 243, 218);color:darkgoldenrod;width:100%;">
						공제가입 사실증명서 ( <%=reqYear %>년 <%=reqMonth %>월 통계 )
					</span>
				</div>
				<div class="box-content padded">
		...
		<div class="col-md-12 padded"> 
			<div class="box">
				<div class="box-header">
					<span class="title" style="font-size:15px;font-weight:bolder;text-align:left;background-color:rgb(255, 243, 218);color:darkgoldenrod;width:100%;">
						공제계약(경력)증명서 ( <%=reqYear %>년 <%=reqMonth %>월 통계 )
					</span>
				</div>
				<div class="box-content padded">
		...
		<div class="col-md-12 padded"> 
			<div class="box">
				<div class="box-header">
					<span class="title" style="font-size:15px;font-weight:bolder;text-align:left;background-color:rgb(255, 243, 218);color:darkgoldenrod;width:100%;">
						적재물가입 사실증명서 ( <%=reqYear %>년 <%=reqMonth %>월 통계 )
					</span>
				</div>
				<div class="box-content padded">
		...
		<script>
			<%
				// 
				Map<String, Map<String, Object>> rMapBun = (Map<String, Map<String, Object>>)request.getAttribute("rMapBun");
				Map<String, Map<String, Object>> rMapGongJoin = ( Map<String, Map<String, Object>>)request.getAttribute("rMapGongJoin");
				Map<String, Map<String, Object>> rMapGongCareer = (Map<String, Map<String, Object>>)request.getAttribute("rMapGongCareer");
				Map<String, Map<String, Object>> rMapMul = (Map<String, Map<String, Object>>)request.getAttribute("rMapMul");
			%>
						...
						data : [
							<%		         
								for(int l=1;l<=lastDay;l++){
	                    			...
	                    			Object rMapB = rMapBun.get(tYM)==null?0+"":rMapBun.get(tYM).get("CNT");
	                    			Object rMapGJ = rMapGongJoin.get(tYM)==null?0+"":rMapGongJoin.get(tYM).get("CNT");
	                    			Object rMapGC = rMapGongCareer.get(tYM)==null?0+"":rMapGongCareer.get(tYM).get("CNT");
	                    			Object rMapMu = rMapMul.get(tYM)==null?0+"":rMapMul.get(tYM).get("CNT");
	                    			if( l>1 ) {
	                       	%> 	
		                                			,
		                    <%			
	                        ...
	                        %>
                        			...
                        			{x:<%="'"+l+"일'"%>, rMapB:<%=rMapB%>, rMapGJ:<%=rMapGJ%>, rMapGC:<%=rMapGC%>, rMapMu:<%=rMapMu%>}
	                        <%	
                        		}
	                        %>
				               ]	
			...
			var chart4 = webponent.chart.init($("#xchart-statistics4"), options, styles, series4);
