...
<c:set var="registerFlag" value="${empty faqBoardVO.articleNo ? '등록' : '수정'}"/>
...
function saveFaqBoard(html,text) {	
    ...
    f.action = "<c:url value="${registerFlag == '등록' ? '/kwa/customer/faqNoticeAdd.do' : '/kwa/customer/faqNoticeModify.do'}"/>";
    f.submit();  
}

/* 글 삭제 funciton*/
function deleteFaqBoard() {
   	document.boardForm.action = "<c:url value='/kwa/customer/faqNoticeDelete.do'/>";
   	document.boardForm.submit();		
}
...
<div class="main-content">
	<form:form commandName="faqBoardVO" name="boardForm" enctype="multipart/form-data">
		<form:hidden path="articleNo"/><br>
		<form:hidden path="atchFileId"/><br>
		<input type="hidden" id="bmstNo" name="bmstNo" value="6"/>
		<input type="hidden" name="content_html"/>
		<input type="hidden" name="content_text"/>
		<input type="hidden" name="searchCondition" value="<c:out value='${searchVO.searchCondition}'/>"/>
		<input type="hidden" name="searchKeyword" value="<c:out value='${searchVO.searchKeyword}'/>"/>
		<input type="hidden" name="pageIndex" value="<c:out value='${searchVO.pageIndex}'/>"/>
		<form:hidden path="bbsId" readonly="true" cssClass="txt" value="FAQ_NOTICE"/><br>
	<div class="container">
		<div class="row">
							...
							<form class="fill-up">
								...
								<div class="box-content">
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
										<code id="contentHtml" style="display:none;">${faqBoardVO.contentHtml}</code>
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
											<c:if test="${not empty faqBoardVO.atchFileId}">
												<c:import url="/com/atch/selectFileInfsForUpdate.do" charEncoding="utf-8">
												<c:param name="param_atchFileId" value="${faqBoardVO.atchFileId}" />
												</c:import>
								 			</c:if> 
								 			<c:if test="${empty faqBoardVO.atchFileId}">
