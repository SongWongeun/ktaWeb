...
function contentsDetail(articleNo){
	...
	document.boardForm.action = "<c:url value='/kwa/customerMgr/praise/detail.do'/>";
   	document.boardForm.submit();
}
</script>
				...
				<div class="box">
					<div class="box-content">
							...
							<tr>
								...
								<td><div id="changViBtn" name="changViBtn" class="btn btn-<c:if test='${result.useAt=="Y"}'>green</c:if><c:if test='${result.useAt=="N"}'>red</c:if>"  style="width: 59px;"><c:if test='${result.useAt=="Y"}'>적용</c:if><c:if test='${result.useAt=="N"}'>미적용</c:if></div></td>
