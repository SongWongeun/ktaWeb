...
<meta property="og:title" content="화물공제조합 - 운전자의 날"/>
...
/* 글 수정 화면 function */
function updateDriverBoard(no) {
	document.boardForm.articleNo.value=no;
   	document.boardForm.action = "<c:url value='/kwa/cyberPr/driversDayUpdate.do'/>";
...
/* 글 삭제 화면 function */
function deleteDriverBoard(no) {
	document.boardForm.articleNo.value = no;
   	document.boardForm.action = "<c:url value='/cyberPr/driversDayDelete.do'/>";
   	document.boardForm.submit();		
}
					...
					<div class="conSection">
						...
						<div class="bbsView1">
							<table class="tbView_st01" summary="제목,작성일,본문내용을 순서로 나타낸 게시물 입니다.">
								<caption>운전자의 날 게시판 상세보기</caption>
								<colgroup>
									<col style="width:11%;" />
									<col style="width:70%;" />
									<col style="width:8%;" />
								...
								<tr class="last">
									...
									<td colspan="4">
										<c:if test="${!empty resultBoard.atchFileId}">
											<c:if test="${not empty resultBoard.atchFileId}">
												<c:import url="/com/atch/selectFileMainInfs.do" charEncoding="utf-8">
