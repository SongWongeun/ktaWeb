...
<c:set var="registerFlag" value="${empty deductBoardVO.articleNo ? '등록' : '수정'}"/>
...
/* 글 등록 function */
function saveDeductBoard(html,text) {	
    ...
    f.action = "<c:url value="${registerFlag == '등록' ? '/kwa/infoLounge/deductNewsAdd.do' : '/kwa/infoLounge/deductNewsModify.do'}"/>";
    f.submit();  
}

/* 글 삭제 funciton*/
function deleteDeductBoard() {
   	document.boardForm.action = "<c:url value='/kwa/infoLounge/deductNewsDelete.do'/>";
   	document.boardForm.submit();		
}
...
<div class="main-content">
	<form:form commandName="deductBoardVO" name="boardForm"  id="boardForm" enctype="multipart/form-data">
		<form:hidden path="articleNo"/><br>
		<form:hidden path="atchFileId"/><br>
		<input type="hidden" id="bmstNo" name="bmstNo" value="1"/>
							...
							<form class="fill-up">
								...
								<c:if test="${not empty ctgList}">
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
											<code id="contentHtml" style="display:none;">${deductBoardVO.contentHtml}</code>
									...
									<c:if test="${empty deductBoardVO.atchFileId}">
										<div class="row box-section">
											<div class="col-md-3">썸네일</div>
											<div class="col-md-9">
												<input name="file_0" id="file_0" type="file" title="첨부파일입력" onchange="checkFileSizeThumb(file_0)"/>
											</div>
										</div>
									</c:if> 
									<div class="row box-section">
										<div class="col-md-3">첨부파일${thumbYn}</div>
										<div class="col-md-9">
											<c:if test="${not empty deductBoardVO.atchFileId}">
												<c:import url="/com/atch/kwaSelectFileInfsForUpdate.do" charEncoding="utf-8">
												<c:param name="param_atchFileId" value="${deductBoardVO.atchFileId}" />
												</c:import>
								 			</c:if> 
								 			<c:if test="${empty deductBoardVO.atchFileId}">
		...
		<ul class="padded separate-sections">
			<li align="right">
				<a href="#;" onclick="saveDeductBoard(CrossEditor.GetBodyValue(),CrossEditor.GetTextValue())"  class="btn btn-blue">
