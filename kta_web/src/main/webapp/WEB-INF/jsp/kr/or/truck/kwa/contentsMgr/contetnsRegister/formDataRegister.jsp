...
<c:set var="registerFlag" value="${empty formDataBoardVO.articleNo ? '등록' : '수정'}"/>
...
/* 글 등록 function */
function saveFormDataBoard(html,text) {
    ...
    f.action = "<c:url value="${registerFlag == '등록' ? '/kwa/customer/formDataAdd.do' : '/kwa/customer/formDataModify.do'}"/>";
    f.submit();  
}

/* 글 삭제 funciton*/
function deleteFormDataBoard() {
   	document.boardForm.action = "<c:url value='/kwa/customer/formDataDelete.do'/>";
   	document.boardForm.submit();		
}
...
<form:form commandName="formDataBoardVO" name="boardForm" enctype="multipart/form-data">
	<form:hidden path="articleNo"/><br>
		<input type="hidden" id="bmstNo" name="bmstNo" value="8"/>
		...
		<form:hidden path="bbsId" readonly="true" cssClass="txt" value="FAQ_NOTICE"/>
		...
		<div class="container">
							...
							<form class="fill-up">
								...
								<c:if test="${not empty ctgList}">
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
										<code id="contentHtml" style="display:none;">${formDataBoardVO.contentHtml}</code>
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
											<c:if test="${not empty formDataBoardVO.atchFileId}">
												<c:import url="/com/atch/selectFileInfsForUpdate.do" charEncoding="utf-8">
												<c:param name="param_atchFileId" value="${formDataBoardVO.atchFileId}" />
												</c:import>
								 			</c:if> 
								 			<c:if test="${empty formDataBoardVO.atchFileId}">
			...
			<ul class="padded separate-sections">
				<li align="right">
					<a href="#;" onclick="saveFormDataBoard(CrossEditor.GetBodyValue(),CrossEditor.GetTextValue())"  class="btn btn-blue">
