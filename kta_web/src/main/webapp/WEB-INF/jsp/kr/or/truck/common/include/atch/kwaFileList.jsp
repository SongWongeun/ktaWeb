	...
	<c:choose>
		<c:when test="${updateFlag=='Y'}">
			...
			<div class="divider"></div>
				<c:choose>
					<c:when test="${thumbYn == 'Y' }">
						<c:if test="${fileNo- fn:length(fileList) + 1 > 0}">
						<div id="fileUpload"><span>파일 업로드</span>
							<div class="row box-section">
								<div class="col-sm-12">
								<c:forEach var="cnt4" begin="1" end="${fileNo- fn:length(fileList) + 1}" >
								...
								<input type="hidden" id="file_0" name="file_0"/>
								</div>
							</div>
						</div>
						</c:if>
