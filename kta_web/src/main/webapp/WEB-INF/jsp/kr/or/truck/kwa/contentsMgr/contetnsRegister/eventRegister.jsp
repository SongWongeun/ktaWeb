...
<c:set var="registerFlag" value="${empty eventBoardVO.articleNo ? '등록' : '수정'}"/>
...
/* 글 등록 function */
function saveEventBoard(html,text) {
	f = document.boardForm;
	f.content_html.value = html.replace(/\n/g,'').replace(/\r/g,'');
	f.content_text.value = text;
	var file = $("form#boardForm input[name='file_1']").val();
	x = f.title.value;
	if (x==null || x==""){
		alert("제목을 입력해 주세요");
		f.title.focus();
    ...
    f.action = "<c:url value="${registerFlag == '등록' ? '/kwa/cyberPr/eventAdd.do' : '/kwa/cyberPr/eventModify.do'}"/>";
    f.submit();  
}

/* 글 삭제 funciton*/
function deleteEventBoard() {
   	document.boardForm.action = "<c:url value='/kwa/cyberPr/eventDelete.do'/>";
   	document.boardForm.submit();		
}
...
<div class="main-content">
	<form:form commandName="eventBoardVO" name="boardForm"  id="boardForm" enctype="multipart/form-data">
		<form:hidden path="articleNo"/><br>
		<form:hidden path="atchFileId"/><br>
		<input type="hidden" id="bmstNo" name="bmstNo" value="13"/>
	...
	<div class="container">
							...
							<form class="fill-up">
								...
								<c:if test="${not empty ctgList}">
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
										<code id="contentHtml" style="display:none;">${eventBoardVO.contentHtml}</code>
									...
									<div class="row box-section">
										...
										<div class="col-md-9">
											<c:if test="${not empty eventBoardVO.atchFileId}">
												<c:import url="/com/atch/selectFileInfsForUpdate.do" charEncoding="utf-8">
												<c:param name="param_atchFileId" value="${eventBoardVO.atchFileId}" />
												</c:import>
								 			</c:if> 
								 			<c:if test="${empty eventBoardVO.atchFileId}">
			...
			<ul class="padded separate-sections">
				<li align="right">
					<a href="#;" onclick="saveEventBoard(CrossEditor.GetBodyValue(),CrossEditor.GetTextValue())"  class="btn btn-blue">
