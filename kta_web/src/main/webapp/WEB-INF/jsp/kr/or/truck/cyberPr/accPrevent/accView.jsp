...
<meta property="og:title" content="화물공제조합 - 사고예방활동"/>
...
/* 글 수정 화면 function */
function updateAccBoard(no) {
	document.boardForm.articleNo.value=no;
   	document.boardForm.action = "<c:url value='/kwa/cyberPr/accUpdate.do'/>";
...
/* 글 삭제 화면 function */
function deleteAccBoard(no) {
	document.boardForm.articleNo.value = no;
   	document.boardForm.action = "<c:url value='/cyberPr/accDelete.do'/>";
   	document.boardForm.submit();		
}
					...
					<div class="conSection">
						...
						<div class="bbsView1">
							<table class="tbView_st01" summary="제목,작성일,본문내용을 순서로 나타낸 게시물 입니다.">
								<caption>사고예방활동목록 읽기게시판</caption>
								<colgroup>
									<col style="width:11%;" />
									<col style="width:70%;" />
									<col style="width:8%;" />
							...
							<div class="btnboxR clfix">
								<span class="btn_pack btn_type01 icon"><span class="check"></span><a href="#;" onclick="listAccBoard()">목록</a></span>
							</div>
						</div>
						</fieldset>
						</form:form>
