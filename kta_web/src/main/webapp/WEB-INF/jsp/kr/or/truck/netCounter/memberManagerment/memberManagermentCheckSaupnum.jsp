...
$(document).ready(function(){
	...
	$('a#certSubmit').on('click',function(e){
		...
		$.ajax({
			url: '/netCounter/memberManagerment/memberManagermentCheckSaupnumCheck.do',
			dataType: "html",
			data : {
    	         "jumin1" : jumin1
               , "jumin2" : jumin2
			...
			success : function( e ){
				...
				if(e=="truetrue"){
					...
					if(s=='NoJohap'){
						//$("#checkSaupList").html("<td colspan='3'>추가 할 수 있는 조합원이 없습니다.</td>");
						alert("가입 된 정보와 동일한 사업자번호를 가진 조합원을 추가 할 수 없습니다.");
					}else{
					
					 $(JSON.parse(s)).each(function(){
						 var AGENT_NM = this.agent_nm;
						 var GYEYAK_JIBU_NM = this.gyeyak_jibu_nm;
						 var AGENT_NO = this.agent_no;
						 var SAUPJANG_SEQ = this.saupjang_seq;
						 var GYEYAK_JIBU_CD = this.gyeyak_jibu;
						 var SAUPNUM = this.saupnum;
					...
					alert("대표자 정보와 추가하려는 사업자정보가 일치하지 않습니다.");
				}
			}
		});
	});
	...
	$("#cancelJohapInsert").on("click",function(){
		$('#typePopupLayer').hide();
	});
	
	$("#JohapInsert").on("click",function(){
		var v_agent_no = "";
		var v_saupjang_seq="";
		var v_gyeyak_jibu="";
		var v_saupnum="";
		$("input:checkbox[name='checkInfo']:checked").each(function(){
			v_agent_no += $(this).attr("attr_agent_no")+"|";
			v_saupjang_seq += $(this).attr("attr_saupjang_seq")+"|";
			v_gyeyak_jibu += $(this).attr("attr_gyeyak_jibu")+"|";
			v_saupnum += $(this).attr("attr_saupnum")+"|";
			
		});
		document.f_insertJohapwon.agent_no.value=v_agent_no;
		document.f_insertJohapwon.saupjang_seq.value=v_saupjang_seq;
		document.f_insertJohapwon.gyeyak_jibu.value=v_gyeyak_jibu;
		document.f_insertJohapwon.saupnum.value=v_saupnum;
		if(""==v_agent_no){
			alert("가입할 조합원을 선택해주세요.");
		}else{
			document.f_insertJohapwon.submit();
		}
...
function  addInfo(AGENT_NM
	...
	list_tbody += "<tr><td>"+AGENT_NM+"</td>";
	list_tbody += "<td>"+GYEYAK_JIBU_NM+"</td>";
	list_tbody += "<td><input attr_agent_no='"+AGENT_NO+"'attr_saupjang_seq='"+SAUPJANG_SEQ+"'attr_gyeyak_jibu='"+GYEYAK_JIBU_CD+"'attr_saupnum='"+SAUPNUM+"' type='checkbox' name='checkInfo'/></td></tr>";
				...
				<div class="conaArea">
					...
					<div class="certifyArea">
						...
						<div class="roundBox certifyBox">
							...
							<div class="infoCheck">
								<h3>사업자 번호 확인</h3>
								<form id="f" name="f" method="post" action="/netCounter/memberManagerment/memberManagermentInsert.do">
								<ul>
									<li>
										<label>
							...
							<input type="hidden" name="jumin1" value="${mVO.jumin1 }" />
							<input type="hidden" name="jumin2" value="${mVO.jumin2 }" />
		 ...
		 <div id="typePopupLayer" style="display:none;">
			<div class="layerBox">
				<div class="layerContents">
					...
					<div class="layerA" style="padding:0px !important;">
						<form method="post" name="f_insertJohapwon" action="/netCounter/memberManagerment/memberManagermentInsert.do">
							<table class="tbIn_st01">
								...
								<tbody id="checkSaupList">
								</tbody>
							</table>
							<div class="btnboxC clfix">
								<span class="btn_pack btn_type16"><a id="JohapInsert" href="#;">추가</a></span>
								<span class="btn_pack btn_type16"><a id="cancelJohapInsert" href="#;">취소</a></span>
							</div>
							<input type="hidden" name="agent_no" value="" />
							<input type="hidden" name="saupjang_seq" value="" />
							<input type="hidden" name="gyeyak_jibu" value="" />
