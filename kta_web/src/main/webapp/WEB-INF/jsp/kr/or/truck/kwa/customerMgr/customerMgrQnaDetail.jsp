...
function backtoIndex(){
	document.boardForm.action = "<c:url value='/kwa/customerMgr/qna/list.do'/>";
	...
	var sawonNo = document.boardForm.forwardId.value;
	  ...
	  if(!CrossEditor.IsDirty()){ 
		...
		if(confirm("답변을 완료하시겠습니까?")){
			document.boardForm.action = "<c:url value='/kwa/customerMgr/qna/contentsUpdate.do'/>";
   			document.boardForm.submit();
		}
}
/*
...
function contentsRepModify(){
	...
	if(confirm("담당자를 변경 하시겠습니까?")){
		document.boardForm.action = "<c:url value='/kwa/customerMgr/qna/contentsRepUpdate.do'/>";
		document.boardForm.submit();
	}
}
...
function contentsPrint(){
	var sURL = '/kwa/customerMgr/qna/detail.do?isPrint=Y&articleNo=${resultDetail.articleNo}';
									...
									<div class="box-content">
										<table class="table ">
											...
											<tbody>
												...
												<tr>
													<td style="background: #F3F4F8;">공개여부</td>
													<td colspan="3" style="text-align:left;">
														<c:choose>
															<c:when test="${resultDetail.open_Yn=='Y'}">
																공개
															</c:when>
															<c:when test="${resultDetail.open_Yn=='N'}">
																비공개
															</c:when>
														</c:choose>
