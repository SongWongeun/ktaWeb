...
function FnSelectFrom(selection,bbsSeq){
	if(selection=="R"){
		...
		$.ajax({
			async : true
	      , type: 'POST'
	      , url: "<c:url value='/kma/bbs/selectVersion.do'/>"
	      , dataType : 'json'
	      , data : {
	    	         "_bbsManagerId" : "${_param._bbs.BBS_MANAGER_ID}"
	               , "_bbsSeq"       : bbsSeq
...
function FnAddFile(){
	...
	fileStr.append("		<a id='BtnFile"+curFileCnt+"' href=javascript:FnDeleteFile('TrFile"+curFileCnt+"','"+curFileCnt+"')>[파일삭제]</a>\n");
	fileStr.append("		</span> \n");
	fileStr.append("	</td> \n");
	fileStr.append("</tr> \n");
...
function FnAddFile(){
	...
	fileStr.append("	<td> \n");
	fileStr.append("		<span id='SpanBtn"+curFileCnt+"'> \n");
	fileStr.append("			<a id='BtnFile"+curFileCnt+"' href=javascript:FnDeleteFile('TrFile"+curFileCnt+"','"+curFileCnt+"') class='btn btn-primary btn-sm' data-role='button' data-inline='true' data-theme='b' >삭제</a>\n");
	fileStr.append("		</span> \n");
	fileStr.append("	</td> \n");
	fileStr.append("</tr> \n");
	// ### 파일 객체추가.
			...
			<div class="col-md-12" >
				<div class="box">
					<div class="box-header">
						<span class="title">검색(버전게시판)</span>
					</div>
								...
								<c:when test="${!empty _param._list}">
									...
									<c:forEach var="data" items="${_param._list}">
										<tr  class="odd" onclick="FnViews('${i}','${data.BBS_SEQ}')" rel="external" style="cursor: pointer;">
											...
											<td class="text-center">${data.REG_DT}</td>
									...
									<c:choose>
										...
										<tr id="rows_by_${i}" ${styles1} class="odd" style="text-align: center;">
											<th class="text-center" style="vertical-align: middle;background-color: white;">내용</th>
											<td colspan="4" class="text-left" style="vertical-align: top;">
